...
describe Spree::Shipment do
  ...
  describe '#finalize_pos' do
    before do
      ...
      @shipment.stub(:state=).with('delivered').and_return(true)
      @shipment.stub(:update_delivered_at).and_return(true)
      @shipment.stub(:save).and_return(true)
    end

    it { @shipment.should_receive(:state=).with('delivered').and_return(true) }
    ...
    it { @shipment.should_receive(:update_delivered_at).and_return(true) }
