class Spree::Admin::PosController < Spree::Admin::BaseController
  ...
  def add
    if pid = params[:item]
      ...
      flash.notice = Spree.t('product_added')
    end
    redirect_to :action => :show 
  end
  ...
  def show
    ...
    if params[:quantity_id] && request.post?
      ...
      @order.reload # must be something cached in there, because it doesnt work without. shame.
      flash.notice = Spree.t("quantity_changed")      
    end
  ...
  def init
    ...
    @order.bill_address = @order.ship_address = user_stock_location(@order.user).first.address
    @order.save!
    method = Spree::ShippingMethod.find_by_name SpreePos::Config[:pos_shipping]
    shipment = @order.shipments.create!
    shipment.stock_location = user_stock_location(@order.user).first
    shipment.save!
    shipment.shipping_methods << (method || Spree::ShippingMethod.first)
    session[:pos_order] = @order.number
  end

  def add_variant var , quant = 1
    init unless @order 
    @order.contents.add(var, quant, nil, @order.shipment)
    #TODO Hack to get the inventory to update. There must be a better way, but i'm lost in spree jungle
    var.product.save
  end

  def user_stock_location(user)
    @stock_location ||= (user.has_spree_role?('admin') ? Spree::StockLocation.active.stores : user.stock_locations.active.store)
  end
